...
**Modèles d'une marque** : noms stockés **par saison** dans `fournisseurs.modeles_by_season` (JSONB `{ [seasonId]: string[] }`). Par magasin (une entrée `parametres` par fournisseur × magasin × saison) : `parametres.modeles` (JSONB `{ [nomModele]: quantité commandée }`) et `parametres.prixModeles` (JSONB `{ [nomModele]: prix HT total }`, colonne SQL `prix_modeles`). Le « reçu » par modèle est calculé depuis `entrees` (somme des `total` groupée par `modele`).

**Prix / PHT des entrées** : le **prix unitaire** d'un modèle = `prixModeles[m] ÷ modeles[m]` (sinon repli sur `parametres.pm`). Dans `EntreeForm`/`EntreeEditModal`, le champ **PHT livré n'est pas saisissable** : il vaut toujours `prix unitaire × quantité reçue` (recalculé quand les tailles changent). Gestion par modèle (quantité + prix) et **import CSV** dans Paramètres → Marques : `parseCsvLine` gère les guillemets/virgules ; seules les colonnes `Marque, Modèle, Qté, PA HT` sont lues (les suivantes ignorées).
