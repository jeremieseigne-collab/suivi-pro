...
- **Sync bidirectionnelle défectueux ↔ SAV** : le statut défectueux et le statut SAV se synchronisent automatiquement. Depuis la liste défectueux (dropdown statut) ou depuis `DefectueuxModal.handleSave` : Mail envoyé → SAV « Mail marque envoyé » ; Avoir reçu / Clôturé / Refusé → SAV « **Clôturé** » (si pas déjà clôturé). Depuis `SavModal` : passage à « Mail marque envoyé » → défectueux « Mail envoyé ». La sync est déclenchée à chaque save sur les statuts déclencheurs (pas seulement au changement).
