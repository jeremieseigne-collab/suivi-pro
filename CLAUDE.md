...
- **Sync bidirectionnelle défectueux ↔ SAV** : le statut défectueux et le statut SAV se synchronisent automatiquement. Depuis la liste défectueux (dropdown statut) ou depuis `DefectueuxModal.handleSave` : Mail envoyé → SAV « Mail marque envoyé » ; Avoir reçu / Clôturé / Refusé → SAV « Réponse reçue » (si pas déjà finalisé). Depuis `SavModal` : passage à « Mail marque envoyé » → défectueux « Mail envoyé ». La sync est déclenchée à chaque save sur les statuts déclencheurs (pas seulement au changement).
- **Liste** : statut affiché en badge coloré ; **clic sur une ligne** ouvre `DefectueuxModal` en édition. Le **dropdown statut** reste dans la liste et déclenche aussi la sync SAV. Boutons ✉️ ✏️ 🗑 avec `stopPropagation`.
...
- **Retour client** (`type: 'retour'`) : statuts Reçu → Mail marque envoyé → Réponse reçue → Clôturé. À la création : auto-crée une entrée « Retour » + un défectueux liés, puis propose d'envoyer un mail au SAV (même flow 3 étapes que DefectueuxModal). Décision finale (quand Clôturé) : Remboursement / Échange / Avoir / **Réparation**. La décision **Réparation** convertit le dossier retour en `type: 'reparation'` sur place (statut → Réponse reçue, décision effacée) et met à jour la note du défectueux lié.
...
- **Réparation** (`type: 'reparation'`) : statuts identiques à Retour client (STATUTS_RETOUR). Toggle **Offert / Payant** (`facturation`) ; si Payant, champ `prixReparation` (colonne `prix_reparation numeric`). À la création : même auto-création entrée Retour + défectueux + mail fournisseur que Retour client. En édition : **pas de champ « Décision »** — remplacé par un **tracker d'avancement** (boutons-étapes stockés dans la colonne `decision`) : Déposé chez le cordonnier → Prête → En attente du client → Client appelé → Récupéré par le client. Quand étape = « Récupéré par le client », le statut passe automatiquement à « Clôturé » à l'enregistrement. L'étape courante s'affiche en violet (🔧) sur la carte.
...
- **Colonnes sav** ajoutées : `prix_reparation numeric`, `en_cours_at timestamptz`, `facturation text`. FIELD_TO_DB : `prixReparation`, `enCoursAt`. La colonne `decision` est réutilisée pour stocker l'étape de réparation (pas de nouvelle colonne).
- **`Sav.jsx`** : liste filtrée par magasin sélectionné + **barre de recherche** (client, marque, modèle, salarié, problème, note) + filtre type (Tous / Retours / Forme / Réparation), barre de progression par statut (`StatusStepper`), timer `FormeTimer` affiché sur les cartes Forme « En cours », stats. Boutons **✉️** (envoie le mail marque + passe le statut SAV à « Mail marque envoyé » + sync défectueux à « Mail envoyé »), **✏️** (édition), **🗑** (suppression avec confirmation) sur chaque carte.
- **Constantes** : `src/sav/constants.js` — `STATUTS_RETOUR`, `STATUTS_FORME`, `DECISIONS` (inclut « Réparation »), `ETAPES_REPARATION`, `STATUT_COLORS`. Mail SAV : `src/sav/mail.js` (`buildSavRetourMailUrl`).
